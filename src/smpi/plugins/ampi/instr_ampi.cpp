...
void TRACE_migration_call(int rank, simgrid::instr::TIData* extra)
{
  ...
  if(smpi_process()->replaying()) {//When replaying, we register an event.
    simgrid::instr::EventType* type =
      static_cast<simgrid::instr::EventType*>(smpi_container(rank)->type_->by_name_or_create<simgrid::instr::EventType>(operation));
    type->add_entity_value(operation, ampi_colors[operation]);
